...
export const setChain = async function (id, store) {
  ...
  if (id === marticId) {
    ...
    web3Process.boxAddress = martic.boxAddress,
    web3Process.buyBoxAddress = martic.buyBoxAddress,
    web3Process.openBoxAddress = martic.openBoxAddress,
    web3Process.nftAddress = martic.nftAddress,
    web3Process.usdtAddress = martic.usdtAddress,
  ...
  if (localStorage.getItem('wallet') === 'Metamask' || localStorage.getItem('wallet') === 'Trust' || localStorage.getItem('wallet') === 'ImToken' || localStorage.getItem('wallet') === 'TokenPocket') {
    try {
      await addNetwork()
      // if (id === bscId) {
      //   store.commit('abis/setNetwork', 'bscscan')
      // }
      // store.commit('wallet/setChangeNetShow', false)
    } catch (e) {
      console.log(e)
      // store.commit('wallet/setNetType', 1)
      // store.commit('wallet/setChangeNetShow', true)
      throw (e)
    }
  } else {
    // if (id === bscId) {
...
export const addNetwork = async function (store) {
  try {
    ...
    // if (localStorage.getItem('wallet') === 'Metamask' || localStorage.getItem('wallet') === 'Trust' || localStorage.getItem('wallet') === 'ImToken') {
          ...
          params: [
            {
                ...
                nativeCurrency: {
                  ...
                  symbol: martic.symbol, // 2-6 characters long
...
export const web3Process = {
  ...
  boxAddress: martic.boxAddress,
  buyBoxAddress: martic.buyBoxAddress,
  openBoxAddress: martic.openBoxAddress,
  nftAddress: martic.nftAddress,
  usdtAddress: martic.usdtAddress,
  approve_amount,
  gas: 5500000,
  gasPrice: '40'
}
